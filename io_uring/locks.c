 ...
 * locks. Only generic VFS managed locks are supported - if a file has
 * a ->flock() or ->lock() file operation, it's punted to that and
 * -EOPNOTSUPP is returned.
 *
 * Lock requests are issued with a single locks_lock_inode() attempt. If
 * a conflicting lock is held, the request is queued on the blocking lock
 * inside fs/locks.c and FILE_LOCK_DEFERRED is returned. Rather than
 * sleeping on the request waitqueue like the sync syscalls do, the
 * request sets ->lm_notify() in the lock manager ops. That callback is
 * invoked when the blocking lock goes away, at which point task_work is
 * queued to retry the lock attempt. A wakeup is not a grant - the retry
 * races with other waiters and may block and defer again, in which case
