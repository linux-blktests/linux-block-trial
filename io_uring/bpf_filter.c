				      ...
				      struct io_kiocb *req)
{
	...
	bctx->buf_index = req->buf_index;
	bctx->resv = 0;
	bctx->user_data = req->cqe.user_data;
	memset(bctx->args, 0, sizeof(bctx->args));
...
static int io_uring_check_bpf_filter(struct sock_filter *filter,
				     unsigned int flen)
{
	int pc;
			   ...
			   struct io_uring_bpf_filter __user *arg)
{
	...
	struct io_bpf_filters *filters;
	struct io_uring_bpf_filter reg;
	...
	if (copy_from_user(&reg, arg, sizeof(reg)))
		return -EFAULT;
	if (reg.opcode >= IORING_OP_LAST)
		return -EINVAL;
	if (reg.flags & ~IO_URING_BPF_FILTER_DENY_REST)
		return -EINVAL;
	if (reg.resv || !mem_is_zero(reg.resv2, sizeof(reg.resv2)))
		return -EINVAL;
	if (!reg.filter_len || reg.filter_len > BPF_MAXINSNS)
		return -EINVAL;

	fprog.len = reg.filter_len;
	fprog.filter = u64_to_user_ptr(reg.filter_ptr);

	ret = bpf_prog_create_from_user(&prog, &fprog, io_uring_check_bpf_filter, false);
	if (ret)
		return ret;
	...
	old_filter = rcu_dereference(filters->filters[reg.opcode]);
	if (old_filter)
		filter->next = old_filter;
	rcu_assign_pointer(filters->filters[reg.opcode], filter);
