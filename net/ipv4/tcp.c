...
void __tcp_cleanup_rbuf(struct sock *sk, int copied)
{
	...
	if (time_to_ack) {
		...
		if (sock_owned_by_user_nocheck(sk) &&
		    READ_ONCE(sock_net(sk)->ipv4.sysctl_tcp_cleanup_ack_defer) &&
...
int tcp_recvmsg(struct sock *sk, struct msghdr *msg, size_t len, int flags)
{
	...
	/*
	 * socket already locked IFF MSG_SOCK_LOCKSTATE is set, and
	 * msg->msg_sock_locked is set. Perform initial napi poll for
	 * initial invocation only.
	 */
	if (!((flags & MSG_SOCK_LOCKSTATE) && msg->msg_sock_locked)) {
